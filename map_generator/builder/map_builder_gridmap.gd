...
func _build_mesh_library(visual_settings: MapVisualSettings) -> MeshLibrary:
	...
	if visual_settings.floor_mesh:
		lib.set_item_mesh(MapGenerator.FLOOR, visual_settings.floor_mesh)
	else:
		var default_floor := BoxMesh.new()
		default_floor.size = Vector3.ONE
		lib.set_item_mesh(MapGenerator.FLOOR, default_floor)
	...
	if visual_settings.wall_mesh:
		wall_mesh = visual_settings.wall_mesh
	else:
		var default_wall := BoxMesh.new()
		default_wall.size = Vector3.ONE
		wall_mesh = default_wall

	lib.set_item_mesh(MapGenerator.WALL, wall_mesh)
